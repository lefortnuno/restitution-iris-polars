  ...
  const exportAsPDF = async () => {
    ...
    try {
      ...
      if (!results || results.length === 0) {
        console.log("Pas de données à exporter.");
        return;
      }
      ...
      const body = [
        keys.map((key) => ({ text: key, bold: true, fillColor: "#eeeeee" })),
        ...results.map((row: any) =>
          keys.map((key) => {
            const raw = row[key];
      ...
      const docDefinition = {
        content: [
          { text: titre, style: "header", margin: [0, 0, 0, 10] },
          {
            table: {
              headerRows: 1,
      ...
      pdfMake.createPdf(docDefinition).download(`${titre}.pdf`);
    } catch (err) {
