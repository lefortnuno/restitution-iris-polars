import React from "react";
import L from "leaflet";
import jsPDF from "jspdf";

interface ExportPDFProps {
  titre: string;
  mapRef: React.RefObject<L.Map | null>;
  legend: {
    label: string;
    value: number;
    color: string;
  }[];
}

export default function ExportMapsPDF({
  titre,
  mapRef,
  legend,
}: ExportPDFProps) {
  const formatCurrency = (value: number): string => {
    if (value === 0) return "0 Dhs";
    const abs = Math.abs(value);
    const sign = value < 0 ? "-" : "";
    if (abs >= 1_000_000_000)
      return `${sign}${(abs / 1_000_000_000).toFixed(1)}B Dhs`;
    if (abs >= 1_000_000) return `${sign}${(abs / 1_000_000).toFixed(1)}M Dhs`;
    if (abs >= 1_000) return `${sign}${(abs / 1_000).toFixed(1)}K Dhs`;
    return `${sign}${abs} Dhs`;
  };

  const createImageWithLegend = async (
    mapImageSrc: string
  ): Promise<HTMLCanvasElement> => {
    return new Promise((resolve) => {
      const canvas = document.createElement("canvas");
      const ctx = canvas.getContext("2d")!;

      const mapImg = new Image();
      mapImg.onload = () => {
        const mapWidth = mapImg.width;
        const mapHeight = mapImg.height;
        const legendHeight = Math.max(100, legend.length * 25 + 60);
        const titleHeight = 60;
        const padding = 20;

        canvas.width = mapWidth + padding * 2;
        canvas.height = mapHeight + legendHeight + titleHeight + padding * 3;

        ctx.fillStyle = "#ffffff";
        ctx.fillRect(0, 0, canvas.width, canvas.height);

        ctx.fillStyle = "#1f2937";
        ctx.font = "bold 24px Arial";
        ctx.textAlign = "center";
        ctx.fillText(titre, canvas.width / 12, padding + 30);

        ctx.drawImage(mapImg, padding, titleHeight + padding);

        const legendStartY = titleHeight + mapHeight + padding * 2;

        ctx.fillStyle = "#374151";
        ctx.font = "bold 16px Arial";
        ctx.textAlign = "left";
        ctx.fillText("Légende:", padding, legendStartY + 20);

        legend.forEach((item, index) => {
          const y = legendStartY + 45 + index * 25;
          ctx.fillStyle = item.color;
          ctx.fillRect(padding + 20, y - 12, 15, 15);

          ctx.strokeStyle = "#666";
          ctx.lineWidth = 1;
          ctx.strokeRect(padding + 20, y - 12, 15, 15);

          ctx.fillStyle = "#374151";
          ctx.font = "14px Arial";
          ctx.fillText(
            `${item.label}: ${formatCurrency(item.value)}`,
            padding + 45,
            y
          );
        });

        resolve(canvas);
      };

      mapImg.src = mapImageSrc;
    });
  };

  const handleExportPDF = async () => {
    if (!mapRef.current) {
      console.error("Map reference is not available");
      return;
    }

    const map = mapRef.current;

    if (!(map as any)._simpleMapScreenshoter) {
      console.error("Simple Map Screenshoter plugin is not initialized");
      return;
    }

    try {
      const screenshoter = (map as any)._simpleMapScreenshoter;

      screenshoter
        .takeScreen("image", {
          mimeType: "image/png",
          quality: 0.9,
        })
        .then(async (mapImage: string) => {
          const canvas = await createImageWithLegend(mapImage);
          const imageData = canvas.toDataURL("image/png");

          const pdf = new jsPDF({
            orientation: canvas.width > canvas.height ? "l" : "p",
            unit: "px",
            format: [canvas.width, canvas.height],
          });

          pdf.addImage(imageData, "PNG", 0, 0, canvas.width, canvas.height);
          pdf.save(`${titre}.pdf`);
        })
        .catch((error: any) => {
          console.error("Error taking screenshot:", error);
          fallbackToHtml2Canvas();
        });
    } catch (error) {
      console.error("Error with screenshoter:", error);
      fallbackToHtml2Canvas();
    }
  };

  const fallbackToHtml2Canvas = async () => {
    try {
      const html2canvas = (await import("html2canvas")).default;
      const mapContainer = mapRef.current?.getContainer();
      if (!mapContainer) return;

      const canvas = await html2canvas(mapContainer, {
        useCORS: true,
        allowTaint: true,
        scale: 2,
      });

      const legendCanvas = await createImageWithLegend(
        canvas.toDataURL("image/png")
      );
      const imageData = legendCanvas.toDataURL("image/png");

      const pdf = new jsPDF({
        orientation: legendCanvas.width > legendCanvas.height ? "l" : "p",
        unit: "px",
        format: [legendCanvas.width, legendCanvas.height],
      });

      pdf.addImage(
        imageData,
        "PNG",
        0,
        0,
        legendCanvas.width,
        legendCanvas.height
      );
      pdf.save(`${titre}_fallback.pdf`);
    } catch (error) {
      console.error("Fallback export also failed:", error);
    }
  };

  return (
    <button
      type="button"
      onClick={handleExportPDF}
      className="inline-flex items-center gap-2 justify-center rounded-md border border-transparent px-3 py-2 text-sm font-medium text-white shadow-sm transition-colors bg-teal-600 hover:bg-teal-700"
    >
      <svg
        version="1.1"
        xmlns="http://www.w3.org/2000/svg"
        viewBox="0 0 550.801 550.801"
        className="w-5 h-5 flex-shrink-0"
        fill="currentColor"
      >
        <path d="M160.381,282.225c0-14.832-10.299-23.684-28.474-23.684c-7.414,0-12.437,0.715-15.071,1.432V307.6c3.114,0.707,6.942,0.949,12.192,0.949C148.419,308.549,160.381,298.74,160.381,282.225z"></path>
        <path d="M272.875,259.019c-8.145,0-13.397,0.717-16.519,1.435v105.523c3.116,0.729,8.142,0.729,12.69,0.729c33.017,0.231,54.554-17.946,54.554-56.474C323.842,276.719,304.215,259.019,272.875,259.019z"></path>
        <path d="M488.426,197.019H475.2v-63.816c0-0.398-0.063-0.799-0.116-1.202c-0.021-2.534-0.827-5.023-2.562-6.995L366.325,3.694c-0.032-0.031-0.063-0.042-0.085-0.076c-0.633-0.707-1.371-1.295-2.151-1.804c-0.231-0.155-0.464-0.285-0.706-0.419c-0.676-0.369-1.393-0.675-2.131-0.896c-0.2-0.056-0.38-0.138-0.58-0.19C359.87,0.119,359.037,0,358.193,0H97.2c-11.918,0-21.6,9.693-21.6,21.601v175.413H62.377c-17.049,0-30.873,13.818-30.873,30.873v160.545c0,17.043,13.824,30.87,30.873,30.87h13.224V529.2c0,11.907,9.682,21.601,21.6,21.601h356.4c11.907,0,21.6-9.693,21.6-21.601V419.302h13.226c17.044,0,30.871-13.827,30.871-30.87v-160.54C519.297,210.838,505.47,197.019,488.426,197.019z M97.2,21.605h250.193v110.513c0,5.967,4.841,10.8,10.8,10.8h95.407v54.108H97.2V21.605z M362.359,309.023c0,30.876-11.243,52.165-26.82,65.333c-16.971,14.117-42.82,20.814-74.396,20.814c-18.9,0-32.297-1.197-41.401-2.389V234.365c13.399-2.149,30.878-3.346,49.304-3.346c30.612,0,50.478,5.508,66.039,17.226C351.828,260.69,362.359,280.547,362.359,309.023z M80.7,393.499V234.365c11.241-1.904,27.042-3.346,49.296-3.346c22.491,0,38.527,4.308,49.291,12.928c10.292,8.131,17.215,21.534,17.215,37.328c0,15.799-5.25,29.198-14.829,38.285c-12.442,11.728-30.865,16.996-52.407,16.996c-4.778,0-9.1-0.243-12.435-0.723v57.67H80.7V393.499z M453.601,523.353H97.2V419.302h356.4V523.353z M484.898,262.127h-61.989v36.851h57.913v29.674h-57.913v64.848h-36.593V232.216h98.582V262.127z" />
      </svg>
      <span className="hidden sm:inline">PDF</span>
    </button>
  );
}
