...
def max_postgres_style(
    data: List[Dict],
    column: str,
    group_by: Optional[Union[str, List[str]]] = None
) -> Union[float, List[Dict]]:
    """
    Calcule le maximum façon PostgreSQL, avec support du GROUP BY.

    :param data: Liste de dicts (données API)
    :param column: Colonne cible pour le MAX
    ...
    if group_by:
        ...
        for key_tuple, values in grouped.items():
            ...
            result.append({**key_dict, column: max_val})
        return result
